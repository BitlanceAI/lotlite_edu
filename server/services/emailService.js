...
const sendEmailAcknowledgement = async (leadData) => {
  ...
  const mailOptions = {
    from: process.env.SMTP_FROM_EMAIL || '"Lotlite SIEC" <no-reply@lotlite.edu>',
    to: email,
    subject: isInternship ? 'Internship Application Received - Lotlite SIEC' : 'Application Received - Lotlite SIEC',
    html: isInternship ? `
      ...
      <br>
      <p>Best regards,</p>
      <p><strong>The Lotlite SIEC Team</strong></p>
    ` : `
      <h3>Hello ${fullName || 'Applicant'}! 🎉</h3>
